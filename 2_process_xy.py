...
# (i) Calculate aggregate cells
df_cells = df_pts.pivot_table(index=['ds','idt'],columns='cell',aggfunc='size')
df_cells = df_cells.fillna(0).astype(int).reset_index()

# (ii) Add on height/width information
df_hw = df_pts.pivot_table(index=['ds','idt'],values=['h','w'],aggfunc='mean').reset_index()
df_cells = df_hw.merge(df_cells)


# (iii) Save aggregate and location information
